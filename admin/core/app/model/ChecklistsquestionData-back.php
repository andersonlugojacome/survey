...
class SurveylistsquestionData
{
    ...
    {
        $sql= "UPDATE ".self::$tablename." SET question=\"$this->question\", description=\"$this->description\",linkpdf=\"$this->linkpdf\", surveylists_id=\"$this->surveylists_id\",user_id=\"$this->user_id\", q_status=\"$this->q_status\", position=\"$this->position\", num_input=\"$this->num_input\"  WHERE id=$this->id ";
        Executor::doit($sql);
    }
    ...
    {
        $sql = "SELECT *, cl.id as cl_id, clq.id as clq_id, clq.question as pregunta  FROM checklists as cl LEFT JOIN ".self::$tablename." as clq ON cl.id = clq.surveylists_id WHERE cl.surveylist_status = '".$mode."' AND cl.id = '".$id."' AND clq.q_status = 'on' ORDER BY clq.position ASC";
    ...
    public static function getAllByChecklistsId($surveylists_id)
    {
