...
class SurveylistsanswerData
{
    ...
    public function add()
    {
        $sql= "INSERT INTO ".self::$tablename." (numeroescriturapublica,ep_anho, answer,checklistsquestions_id, user_id, client_id, created_at, surveylists_id, a_code_approval)";
        $sql .= " VALUES (\"$this->numeroescriturapublica\",\"$this->ep_anho\",\"$this->answer\",\"$this->checklistsquestions_id\",\"$this->user_id\",\"$this->client_id\",\"$this->created_at\",\"$this->surveylists_id\" ,\"$this->a_code_approval\" )";
        Executor::doit($sql);
    }
    ...
    {
        $sql = "SELECT cl.id as cl_id, cla.id as cla_id, cla.checklistsquestions_id as clq_id,  cla.answer as respuesta FROM checklists as cl LEFT JOIN checklistsanswers as cla ON cl.id = cla.surveylists_id WHERE cl.surveylist_status = 'open' AND cl.id = '".$surveylists_id."' AND cla.numeroescriturapublica ='".$numeroescriturapublica."' AND cla.ep_anho='".$anho."' ORDER BY cla.id ASC ";
        $query = Executor::doit($sql);
        return Model::many($query[0], new SurveylistsanswerData());
    }
    public static function getAllAnswers($numeroescriturapublica, $anho, $surveylists_id)
    {
        $sql = "SELECT cl.id as cl_id, cla.id as cla_id, cla.checklistsquestions_id as clq_id, cla.answer as respuesta FROM checklists as cl LEFT JOIN checklistsanswers as cla ON cl.id = cla.surveylists_id WHERE cl.id = '".$surveylists_id."' AND cla.numeroescriturapublica ='".$numeroescriturapublica."' AND cla.ep_anho='".$anho."' ORDER BY cla.id ASC ";
        $query = Executor::doit($sql);
        return Model::many($query[0], new SurveylistsanswerData());
    }
